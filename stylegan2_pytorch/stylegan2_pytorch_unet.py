...
class Encoder(nn.Module):
    def __init__(self, image_size,in_channels=1, network_capacity=64, fmap_max=512):
        super().__init__()
...
class Generator(nn.Module):
    ...
    def forward(self, styles, input_noise, img_in):
        batch_size = styles.shape[0]
        skip_connections = self.encoder(img_in)
        ...
        for i, (style, block, attn) in enumerate(zip(styles, self.blocks, self.attns)):
            if exists(attn):
                x = attn(x)
            if i >= 1:
                # print("X.shape", i, x.shape, skip_connections[i-1].shape)
...
class Trainer():
    ...
    def train(self):
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, S, G]):
            ...
            grayscale_images = image_batch.mean(dim=1, keepdim=True)
            grayscale_images = grayscale_images.detach()
            generated_images = G(w_styles, noise, grayscale_images)
            generated_images = generated_images.detach()
            fake_output = torch.cat((grayscale_images, generated_images), 1)

            fake_output, fake_q_loss = D_aug(fake_output.clone().detach(), detach = True, **aug_kwargs)
            
            image_batch = torch.cat((grayscale_images, image_batch), 1)
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[S, G, D_aug]):
            ...
            # dummy_input = torch.randn(batch_size, 3, image_size, image_size).to('cuda')
            generated_images = G(w_styles, noise, grayscale_images)
            fake_output = torch.cat((grayscale_images, generated_images), 1)
            fake_output, _ = D_aug(fake_output, **aug_kwargs)
            fake_output_loss = fake_output
    ...
    def evaluate(self, num = 0, trunc = 1.0):
        ...
        # Fetch and process grayscale images from the loader
        real_images = next(self.loader).to(self.rank)
        grayscale_images = real_images.mean(dim=1, keepdim=True)
        ...
        # regular
        generated_images = self.generate_truncated(self.GAN.S, self.GAN.G, latents, n, grayscale_images, trunc_psi = self.trunc_psi)
        torchvision.utils.save_image(grayscale_images, str(self.results_dir / self.name / f'grayscale_{str(num)}.{ext}'), nrow=num_rows)
        torchvision.utils.save_image(generated_images, str(self.results_dir / self.name / f'generated_{str(num)}.{ext}'), nrow=num_rows)
        
        # moving averages
        generated_images = self.generate_truncated(self.GAN.SE, self.GAN.GE, latents, n, grayscale_images, trunc_psi = self.trunc_psi)
        torchvision.utils.save_image(generated_images, str(self.results_dir / self.name / f'generated_{str(num)}-ema.{ext}'), nrow=num_rows)
        ...
        generated_images = self.generate_truncated(self.GAN.SE, self.GAN.GE, mixed_latents, n, grayscale_images, trunc_psi = self.trunc_psi)
        torchvision.utils.save_image(generated_images, str(self.results_dir / self.name / f'generated_{str(num)}-mr.{ext}'), nrow=num_rows)
    ...
    @torch.no_grad()
    def generate_truncated(self, S, G, style, noi, img_in, trunc_psi = 0.75, num_image_tiles = 8):
        w = map(lambda t: (S(t[0]), t[1]), style)
        w_truncated = self.truncate_style_defs(w, trunc_psi = trunc_psi)
        w_styles = styles_def_to_tensor(w_truncated)
        generated_images = evaluate_in_chunks(self.batch_size, G, w_styles, noi, img_in)
